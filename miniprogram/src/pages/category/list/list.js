...
const COLORS = ['#1989fa', '#07c160', '#ff976a', '#ed6a0c', '#7232dd', '#07c160', '#ff4d4f'];
...
Page({
  ...
  editCategory(e) {
    ...
    wx.showModal({
      ...
      content: name,
      success: async (res) => {
        if (res.content && res.confirm && res.content !== name) {
          try {
            await api.categories.update(id, { name: res.content });
            showSuccess('更新成功');
            this.loadCategories();
          } catch (error) {
